...
func TestVerifyProofSharedMemory(t *testing.T) {
	log.Println("调用TestVerifyProofSharedMemory")
	seed := []byte{0xde, 0xad, 0xbe, 0xef}
	...
	assert.Equal(t, true, vdf_go.VerifyVDF(seed, output[len(seed):], 10, 2048), "must be true")
}

// func TestGenerateAndVerifyIntSize(t *testing.T) {
// 	log.Println("调用TestGenerateAndVerifyIntSize")
// 	seed := []byte{0xde, 0xad, 0xbe, 0xef}
...
// 	log.Println("调用TestGenerateAndVerifyProof1000")
// 	seed := []byte{0xde, 0xad, 0xbe, 0xef}

// 	// for T := 1001; T < 1010; T++ {
// 	// 	y_buf, proof_buf := vdf_go.GenerateVDF(seed, T, 2048)
// 	// 	assert.Equal(t, true, vdf_go.VerifyVDF(seed, append(y_buf, proof_buf...), T, 2048), "failed when T = %d", T)
// 	y_buf, proof_buf := vdf_go.GenerateVDF(seed, 10000, 2048)
// 	assert.Equal(t, true, vdf_go.VerifyVDF(seed, append(y_buf, proof_buf...), 10000, 2048), "failed when T = %d", 10000)
// 	// }
// }

func TestRandomInput(t *testing.T) {
	log.Println("调用TestRandomInput")
	for i := 0; i < 10; i++ {
		seed := make([]byte, 32)
		rand.Read(seed)
